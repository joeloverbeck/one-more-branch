...
const DEFAULT_STRUCTURE_EVALUATOR_MAX_TOKENS = 8192;
...
async function callStructureEvaluatorStructured(
  ...
  const maxTokens = options.maxTokens ?? DEFAULT_STRUCTURE_EVALUATOR_MAX_TOKENS;
  ...
  if (!content) {
    ...
    logger.warn('Structure evaluator received empty content from OpenRouter', {
      finishReason,
      usage,
      model,
