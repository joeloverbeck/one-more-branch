...
function buildStory(overrides?: Partial<Story>): Story {
  return {
    ...
    decomposedCharacters: [
      buildMinimalDecomposedCharacter('A courier smuggling letters through occupied cities'),
    ],
...
describe('page-service', () => {
  ...
  describe('generateFirstPage', () => {
          ...
          reconciledStateWithDiagnostics(openingWriterResult, '', [
            {
              ...
              field: 'threadsAdded',
              message:
                'Thread add "Reach the archive" is near-duplicate of existing thread "td-1".',
            },
          ])
        )
    ...
    it('passes structure to opening context and uses initial structure state when present', async () => {
      ...
      expect(page.choices.map((choice) => choice.text)).toEqual(['Option A', 'Option B']);
      expect(updatedStory.globalCanon).toContainEqual({
        text: 'The city enforces nightly curfew',
        factType: 'LAW',
      });
  ...
  describe('generateNextPage', () => {
    ...
    it('creates child page with proper parent linkage and sequential id', async () => {
      ...
      const parentStructureState = createInitialStructureState(structure);
      const story = buildStory({
        globalCanon: [{ text: 'The watch captain is corrupt', factType: 'NORM' }],
        structure,
      ...
      expect(updatedStory.globalCanon).toContainEqual({
        text: 'Clocktower guards rotate every ten minutes',
        factType: 'LAW',
      });
      expect(metrics).toEqual(
        expect.objectContaining({
    ...
    it('triggers structure rewrite and creates new version when deviation is detected', async () => {
      ...
      mockedGenerateWriterPage.mockResolvedValue({
        ...
        constraintsAdded: [
          { text: 'Must prove loyalty', constraintType: ConstraintType.ENVIRONMENTAL },
        ],
        constraintsRemoved: [],
        threadsAdded: [
      ...
      expect(updatedStory.globalCanon).toContainEqual({
        text: 'Resistance branded you a traitor',
        factType: 'LAW',
      });
      expect(onGenerationStage.mock.calls).toEqual(
        expect.arrayContaining([
    ...
    describe('pacing response', () => {
      ...
      it('skips pacing logic when deviation is detected', async () => {
        ...
        mockedGenerateWriterPage.mockResolvedValue({
          ...
          threatsAdded: [{ text: 'Hunted by former allies', threatType: ThreatType.ENVIRONMENTAL }],
          threatsRemoved: [],
          constraintsAdded: [],
          constraintsRemoved: [],
          threadsAdded: [],
