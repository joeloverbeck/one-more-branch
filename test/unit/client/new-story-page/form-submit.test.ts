...
describe('new story form submit', () => {
  ...
  function fillForm(overrides: Record<string, string> = {}): void {
    ...
    const worldbuildingEl = document.getElementById('worldbuilding') as HTMLTextAreaElement | null;
    if (worldbuildingEl) worldbuildingEl.value = values.worldbuilding;
    const startingSituationEl = document.getElementById(
      'startingSituation'
    ) as HTMLTextAreaElement | null;
    if (startingSituationEl) startingSituationEl.value = values.startingSituation;
    ...
    // Set protagonist character selector value (required for validation)
    const protagonistSelector = document.getElementById(
      'protagonist-character-selector'
    ) as HTMLSelectElement | null;
    if (protagonistSelector) {
      const option = document.createElement('option');
      option.value = values.protagonistCharacterId;
      option.textContent = 'Test Character';
      protagonistSelector.appendChild(option);
      protagonistSelector.value = values.protagonistCharacterId;
    }
  }
