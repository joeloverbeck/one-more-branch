...
describe('Choice utilities', () => {
  describe('createChoice', () => {
    it('creates a choice with null nextPageId by default', () => {
      ...
      expect(choice.choiceType).toBe(ChoiceType.INTERVENE);
      expect(choice.primaryDelta).toBe(PrimaryDelta.GOAL_PRIORITY_CHANGE);
    });
    ...
    it('creates a choice with custom choiceType and primaryDelta', () => {
      ...
      expect(choice.nextPageId).toBeNull();
      expect(choice.choiceType).toBe(ChoiceType.CONTEST);
      expect(choice.primaryDelta).toBe(PrimaryDelta.THREAT_LEVEL_CHANGE);
    });
    ...
    it('throws for invalid choiceType', () => {
      expect(() => createChoice('Go', null, 'INVALID_TYPE' as ChoiceType)).toThrow(
        'Invalid choiceType'
      );
    });
    ...
    it('throws for invalid primaryDelta', () => {
      expect(() =>
        createChoice('Go', null, ChoiceType.INTERVENE, 'INVALID_DELTA' as PrimaryDelta)
  ...
  describe('isChoice', () => {
    ...
    it('returns false when choiceType is missing', () => {
      expect(
        isChoice({ text: 'Go', primaryDelta: PrimaryDelta.GOAL_PRIORITY_CHANGE, nextPageId: null })
      ).toBe(false);
    });
