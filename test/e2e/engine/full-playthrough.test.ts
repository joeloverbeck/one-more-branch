...
const openingResult = {
  ...
  stateChanges: ['Sibling hidden in cellar', 'Time fractures detected at district boundary'],
  canonFacts: ['Districts shift at midnight based on beacon towers', 'Patrols use searchlights in fog'],
  characterCanonFacts: {},
  isEnding: false,
...
function buildContinuationResult(selectedChoice: string, stepIndex: number): typeof openingResult {
  if (selectedChoice.includes('beacon tower')) {
    return {
      ...
      stateChanges: ['Reached beacon tower', 'Patrol spotted approaching'],
      canonFacts: ['Beacon towers emit temporal energy'],
      characterCanonFacts: {},
      isEnding: false,
      storyArc: 'Map the time fractures while keeping your sibling safe from patrols.',
  ...
  if (selectedChoice.includes('patrol schedules') || selectedChoice.includes('checkpoint')) {
    return {
      ...
      stateChanges: ['Infiltrated checkpoint', 'Learned patrol routes'],
      canonFacts: ['Checkpoints store patrol schedules on physical boards'],
      characterCanonFacts: {},
      isEnding: false,
      storyArc: 'Map the time fractures while keeping your sibling safe from patrols.',
  ...
  if (selectedChoice.includes('hiding spot') || selectedChoice.includes('stalled district')) {
    return {
      ...
      stateChanges: ['Entered stalled district', 'Found time-dilated warehouse'],
      canonFacts: ['Stalled districts experience severe time dilation'],
      characterCanonFacts: {},
      isEnding: false,
      storyArc: 'Map the time fractures while keeping your sibling safe from patrols.',
  ...
  return {
    ...
    stateChanges: ['Patrol alerted', 'Critical decision point reached'],
    canonFacts: ['Patrols can track movement through time fractures'],
    characterCanonFacts: {},
    isEnding: false,
    storyArc: 'Map the time fractures while keeping your sibling safe from patrols.',
...
const replayOpeningResult = {
  ...
  stateChanges: ['Carrying testimony documents', 'Canal waters rising'],
  canonFacts: ['Canals respond to civic votes at dawn sirens', 'Political factions alter street reality'],
  characterCanonFacts: {},
  isEnding: false,
...
function buildReplayContinuationResult(): ReturnType<typeof buildContinuationResult> {
  return {
    ...
    stateChanges: ['Passed under the bridge undetected', 'Documents still secure'],
    canonFacts: ['Safe houses have underwater access points'],
    characterCanonFacts: {},
    isEnding: false,
    storyArc: 'Deliver the testimony before the factions discover you.',
