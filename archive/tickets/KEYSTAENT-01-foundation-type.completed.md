...
- **Changes**:
  - Created `src/models/state/keyed-entry.ts` with `KeyedEntry` interface, `StateIdPrefix` type, and 5 utility functions
  - Created `test/unit/models/state/keyed-entry.test.ts` with all 14 acceptance criteria tests
  - Deleted `src/models/state/tagged-entry.ts`
- **Deviations**: None
- **Verification**: `npx tsc --noEmit keyed-entry.ts` clean, 14/14 tests passing
