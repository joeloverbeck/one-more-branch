...
2. `src/server/routes/chat.ts` currently uses `resolveStatusCode(error)` with `error.message.includes(...)` checks for cross-layer failures returned from the chat stack. That is the exact architectural weakness left after CHACHASYS-012.
...
4. Existing repository patterns already justify typed error contracts for cross-layer orchestration:
   ...
   - `src/server/routes/character-webs.ts` and `src/services/character-web-service.ts` already branch on typed `EngineError` codes instead of string parsing.
...
6. Current tests only assert string messages for chat failures. There is not yet direct coverage proving code-based route mapping or typed error contracts in chat service/repository/pipeline/validation.
7. `updateChat()` in `src/persistence/chat-repository.ts` is the only current path that throws the repository's "missing chat" and "ID mismatch" update errors. Those cases are not covered by existing repository tests yet.
...
3. Request-shape validation that is fully local to the route can remain route-local and does not need to be forced through the domain-error contract. Once an error crosses a boundary between chat layers, it should be typed.
...
- `src/persistence/chat-repository.ts`
  - missing chat on `updateChat()`
  - chat ID mismatch on `updateChat()`
- `src/llm/chat/chat-pipeline.ts`
...
- Do not route-localize or rework request parsing failures in `src/server/routes/chat.ts` that never cross into the chat domain, such as missing POST body fields, enum parsing failures, message length checks, or missing API key checks. Those should remain simple route validation responses unless a broader app-wide request-validation architecture is introduced in a separate ticket.
...
- repository tests that cover `updateChat()` missing-chat and ID-mismatch failures, since those cases are not currently exercised

## Files to Touch

- `tickets/CHACHASYS-018-typed-chat-domain-errors.md` (modify before implementation)
...
- `test/unit/models/chat/chat-validation.test.ts` (new or modify, recommended if no existing validation-focused file covers the parser surface cleanly)
...
3. Unit test: chat repository throws typed errors for `updateChat()` missing-chat, `updateChat()` ID mismatch, and invalid persisted payloads
...
3. `test/unit/persistence/chat-repository.test.ts` — covers typed `updateChat()` failures plus persisted corruption failures
4. `test/unit/llm/chat/chat-pipeline.test.ts` — covers typed invariant failure
5. `test/unit/models/chat/chat-validation.test.ts` — covers typed invalid-persisted-data parsing failures if validation assertions are split out
6. `test/unit/models/chat/chat-errors.test.ts` — covers the new domain error contract itself

### Commands

1. `npm run test:unit -- --testPathPatterns='chat|chat-repository'`
2. `npm run typecheck`
3. `npm run lint`
4. `npm test`
...
- Actual changes:
  - Added a canonical `ChatDomainError` contract in `src/models/chat/chat-errors.ts` and re-exported it from the chat model surface.
  - Replaced cross-layer plain `Error` throws in chat service, repository, pipeline, and persisted-payload validation with typed chat-domain errors.
  - Removed message-fragment route mapping from `src/server/routes/chat.ts` and replaced it with explicit status mapping for `ChatDomainError`.
  - Added a small route-local `RouteValidationError` for HTTP-body parsing failures that never enter the chat domain, keeping request validation separate from domain failures.
  - Strengthened route, service, repository, pipeline, and model tests around the typed contract.
- Deviations from original plan:
  - No separate `test/unit/models/chat/chat-validation.test.ts` file was needed; persisted-payload validation coverage now lives in strengthened repository tests, which exercise the real repository entry points that surface those errors.
  - The route layer now distinguishes route-local validation from domain validation explicitly instead of forcing request-shape failures through `ChatDomainError`.
- Verification results:
  - `npm run test:unit -- --testPathPatterns='chat|chat-repository'`
  - `npm run typecheck`
  - `npm run lint`
  - `npm test`
