...
- `toneKeywords` and `toneAntiKeywords` are now generated by the spine (not the structure generator). They are stored on the `Story` model and propagated to all downstream prompts for tone consistency.
- The structure generator receives decomposed character profiles and world facts (from the entity decomposer) plus the story spine, rather than raw prose.
