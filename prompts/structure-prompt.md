...
- When `conceptSpec.genreFrame` is present, two genre-related sections are injected before the thematic kernel: a **GENRE CONVENTIONS** block (from `buildGenreConventionsSection()` in `src/llm/prompts/sections/shared/genre-conventions-section.ts`) listing 6 persistent atmospheric/tonal constraints as `- tag: gloss` entries, and a **GENRE OBLIGATION CONTRACT** block (from `buildGenreObligationsSection()`) listing 6 required scene-type obligations as `- tag: gloss` entries. Obligations use the expanded `{ tag, gloss }` format (not flat strings).
