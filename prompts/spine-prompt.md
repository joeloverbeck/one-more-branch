...
When `decomposedCharacters` are provided (from the character dev pipeline), the prompt has access to pre-analyzed structured character profiles. When a `conceptVerification` is provided, the prompt includes signature scenario and inevitability constraints.
