...
The setpiece TEXT generated by this stage is **verification-internal only**. It is used to compute `conceptIntegrityScore` and `setpieceCausalChainBroken`, but the actual setpiece strings are NOT propagated to `ConceptVerification` or passed downstream.

The architecture pipeline (`MacroArchitectureResult.setpieceBank`) generates its own setpiece bank with richer context (decomposed characters, world, spine, kernel).
...
- The combined output from both stages is assembled by `combineVerifications()` in `concept-verifier.ts` into the `ConceptVerification` shape. Note: `escalatingSetpieces` and `setpieceCausalLinks` are deliberately omitted from `ConceptVerification` — only `setpieceCausalChainBroken` and `conceptIntegrityScore` are propagated as verification diagnostics.
