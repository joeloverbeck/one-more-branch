...
5. The current module split is mismatched with actual ownership:
   ...
   - taste/spark/packeter/evaluator request-result contracts are another
...
2. The clean split is:
   - a projection-focused module for `ConceptSeedPacket` and projection helpers
   - a taxonomy module for shared enums/guards/constants
   - a generation-contract module for taste/spark/one-shot/packeter/evaluator request-result shapes
3. This is more robust than leaving all content-domain contracts in one file because downstream imports become explicit about which subdomain they depend on, which reduces accidental coupling and makes later changes safer.
4. No backwards-compatibility shims, alias paths, or duplicate exports should be introduced. Old module paths should be removed once imports are migrated.
...
- `concept-seed-packet.ts`
  ...
  - projection and validation helpers specific to those contracts
...
1. `test/unit/models/content-packet.test.ts` — migrate to the new projection/taxonomy module ownership and keep projection helper coverage intact.
2. `test/unit/models/saved-content-packet.test.ts` — ensure saved-asset validation still composes correctly with the moved projection helpers.
3. `test/unit/llm/content-packeter.test.ts` — ensure generation parser imports/types still resolve after contract-module extraction.
4. `test/unit/server/services/content-service.test.ts` — ensure service composition still works when imports are split across focused modules.
5. `test/unit/llm/prompts/concept-*.test.ts` and verifier/evolver tests — ensure concept-stage consumers still compile and render from the dedicated projection module.

### Commands

1. `npm run test:unit -- --runTestsByPath test/unit/models/content-packet.test.ts test/unit/models/saved-content-packet.test.ts test/unit/llm/content-packeter.test.ts test/unit/server/services/content-service.test.ts test/unit/server/services/content-packet-artifact.test.ts test/unit/server/presenters/content-packet-card.test.ts test/unit/server/routes/content-packets-routes.test.ts test/unit/llm/prompts/concept-seeder-prompt.test.ts test/unit/llm/prompts/concept-architect-prompt.test.ts test/unit/llm/prompts/concept-engineer-prompt.test.ts test/unit/llm/prompts/concept-evolver-seeder-prompt.test.ts test/unit/llm/prompts/concept-verifier-prompt.test.ts test/unit/llm/concept-evolver.test.ts test/unit/llm/concept-verifier.test.ts test/unit/llm/concept-stress-tester.test.ts`
2. `npm run typecheck`
3. `npm run lint`
