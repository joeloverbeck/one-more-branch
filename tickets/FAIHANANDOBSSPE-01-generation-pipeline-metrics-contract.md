...
3. Add a generation result-level field (engine-facing contract) for metrics so `generateFirstPage()` and `generateNextPage()` can return/forward structured metrics without loose dictionaries.
